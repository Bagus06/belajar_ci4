...
class ExampleModel extends Model
{
    ...
    public function exits($item = 0)
    {
        $ret  = false;
        $query = "SELECT * FROM " . $this->table1;
        if (empty($this->primary_column)) {
            return "primary column cannot be empty.";
        }
    ...
    public function example($id = 0, $datas = "", $type = "")
    {
        $ret = false;
        if (empty($type)) {
        ...
        if (strtoupper($type) === "POST") {
            ...
            if ($this->db->query($sql)) {
                $ret = [
                    "response" => true,
                    "response_message" => "Success to insert data."
                ];
            } else {
                $ret = [
                    "response" => false,
            ...
            if (!empty($id)) {
                ...
                if ($this->db->query($sql)) {
                    $ret = [
                        "response" => true,
                        "response_message" => "Success to set data."
                    ];
                } else {
                    $ret = [
                        "response" => false,
                ...
                $ret = [
                    "response" => false,
                    "response_message" => "ID cannot empty."
                ];
            }
    ...
    public function next_previous_edit($id = 0)
    {
        $ret = [];
        $query_next = "";
        $next = false;
        ...
        if (!empty($id)) {
            $id = decrypt_url($id);
            $query_next .= "SELECT " . $this->table1 . ".id FROM " . $this->table1 . " WHERE " . $this->table1 . ".id > " . $id . $this->whereclause_system() . " ORDER BY "  . $this->table1 . ".id ASC LIMIT 1";
            $query_prev .= "SELECT " . $this->table1 . ".id FROM " . $this->table1 . " WHERE " . $this->table1 . ".id < " . $id . $this->whereclause_system() . " ORDER BY "  . $this->table1 . ".id DESC LIMIT 1";

            $next = $this->db->query($query_next)->getRowArray();
            if ($next) {
                $ret["next"] = ($next["id"] > $id) ? $next["id"] : false;
            }

            $prev = $this->db->query($query_prev)->getRowArray();
            if ($prev) {
                $ret["prev"] = ($prev["id"] < $id) ? $prev["id"] : false;
            }
        }
